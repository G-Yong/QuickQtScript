...
void MainWindow::abortCurrentScript()
{
    ui->pushButton_start->setVisible(true);
    ui->pushButton_stop->setVisible(false);
    ui->pushButton_stepIn->setVisible(false);
            ...
            else
            {
                ...
                if(mActiveRequestedRunToLine > 0)
                {
                    ...
                    else
                    {
                        ...
                        // 目标函数和行号由 engine/QuickJS 校验；agent 只接收命中通知。
...
{
    /* requestPause 原子判断并提交 Running -> Requested。若已经请求或已经
       暂停，第二次点击“停止”沿用原行为，转为真正终止当前脚本。 */
    if(mEngineAgent.isNull() == false && mEngineAgent->requestPause())
        return;
...
QScriptValue funcSleep(QScriptContext *context, QScriptEngine *engine, void *data)
{
    ...
    {
        // 每轮循环都是 funcSleep 可安全恢复的位置。
        engine->pauseCheckpoint();
